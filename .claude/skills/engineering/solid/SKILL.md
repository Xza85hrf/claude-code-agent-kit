---
name: solid
description: Writing or reviewing code quality. Use when applying SOLID principles, refactoring for maintainability, reviewing design decisions, or ensuring professional code craftsmanship.
argument-hint: "Review the UserService class for SOLID violations and suggest refactoring"
...
Every line of code must embody professional craftsmanship. Code enables developers to discover, understand, add, change, remove, test, debug, deploy, and monitor features efficiently.
...
- Wrap primitives in domain objects (IDs, emails, money amounts)
...
class Money { constructor(private readonly amount: number, currency: string) {} }
...
**Context:** `OrderProcessor` handles validation, price calculation, inventory, AND audit logs. New requirement: log to remote service instead of file.

**Smell detection:**
- Class does too many things (SRP violation)
- Changing log destination requires modifying `OrderProcessor`

**Refactoring:**
1. Extract a `Logger` interface: `log(event: AuditEvent): void`
2. Create `FileLogger` and `RemoteLogger` implementations
3. Inject `Logger` into `OrderProcessor` (DIP)
4. `OrderProcessor` calls `this.logger.log(event)`

**Result:**
- New log destinations require zero changes to `OrderProcessor`
- Each class has one reason to change

**Diagnostic signal:** If you cannot name a class after everything it does, it is doing too much.
